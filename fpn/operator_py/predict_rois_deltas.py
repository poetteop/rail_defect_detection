...
class PredRoisDeltasOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if is_train:
            ...
            with autograd.train_section():
                for i in range(self.num_strides):
                    self.roi_pool[i] = mx.nd.ROIPooling(in_data[i], mx.nd.array(rois_p[i], in_data[i].context), (7, 7),
                                                        spatial_scale=1.0 / self.feat_strides[i])

                    rpn_conv = mx.sym.Convolution(data=in_data[i], kernel=(3, 3), pad=(1, 1), num_filter=512,
...
@mx.operator.register('predict_rois_deltas')
class PredRoisDeltasProp(mx.operator.CustomOpProp):
    def __init__(self, feat_strides='(4,8,16,32)', pooled_height='7', pooled_width='7', output_dim='256'):
        super(PredRoisDeltasProp, self).__init__(need_top_grad=True)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return PredRoisDeltasOperator(self.feat_strides, self.pooled_height, self.pooled_width, self.output_dim, self.with_deformable)
