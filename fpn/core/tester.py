...
class Predictor(object):
    ...
    def pred_raw_data(predictor, data_batch, data_names, scales, cfg, scores_field='cls_prob_reshape', imdb=None):
        ...
        return rois,fc_new_1
            # rps_fname = '/dccstor/jsdata1/dev/RepMet/output/dev/test_fc_new_1_store.pkl'
            # with open(rps_fname, 'wb') as fid:
            #     cPickle.dump(fc_new_1, fid, protocol=cPickle.HIGHEST_PROTOCOL)
            #

def im_detect(predictor, data_batch, data_names, scales, cfg, scores_field='cls_prob_reshape', imdb=None):
    if predictor is None:
        import requests

        IMG_PATH = imdb.roidb[data_batch.index]['image']
        old_path,fname = os.path.split(IMG_PATH)
        IMG_PATH = os.path.join('/vol1/test_images/JES_pilot',fname)
        data_dict_all = None
        SERVER_URL = "http://localhost:5000"
        url = "{}/detect".format(SERVER_URL)
        score_thresh = 0.01
        num_candidates = 1
        # reply = requests.post(url, data=files)
        reply = requests.post(url, files={'image': open(IMG_PATH, 'rb')}, data={'score_thresh': score_thresh, 'num_candidates': num_candidates})

        num_classes = cfg.dataset.NUM_CLASSES-1

        scores = np.zeros((0,num_classes))
        pred_boxes = np.zeros((0,4))
        for object in reply.json():
            label = object['labels'][0]['label']
            score = float(object['labels'][0]['score'])
            ord = int(object['labels'][0]['order'])
            top = int(object['object_location']['top'])
            left = int(object['object_location']['left'])
            height = int(object['object_location']['height'])
            width = int(object['object_location']['width'])
            bottom = top + height
            right = left + width
            s = np.zeros((1,num_classes))
            s[ord-1] = score
            scores = np.concatenate((scores,s),axis = 0)
            b=np.array([left,top,right,bottom])
            pred_boxes = np.concatenate((pred_boxes, b), axis=0)
...
def im_detect_feats(predictor, data_batch, data_names, scales, cfg, scores_field='cls_prob_reshape'):
    output_all = predictor.predict(data_batch)
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        ...
        scores = output[scores_field+'_output'].asnumpy()[0]
        bbox_deltas = output['bbox_pred_reshape_output'].asnumpy()[0]
        rois_feats = output['psp_final_embed_output'].asnumpy()  # shape: [#rois, Embed_dim]
        rois_feats_all.append(rois_feats)
        if False:
            roi_pool= output['roi_pool_output'].asnumpy()  # shape: [#rois, Embed_dim]
        # post processing
        pred_boxes = bbox_pred(rois, bbox_deltas)
        pred_boxes = clip_boxes(pred_boxes, im_shape[-2:])
        ...
        scores_all.append(scores)
        pred_boxes_all.append(pred_boxes)
    return scores_all, pred_boxes_all, data_dict_all,rois_feats_all


def im_detect_feats_stats(predictor, data_batch, data_names, scales, cfg,stat_type, scores_field='cls_prob_reshape'):
    output_all = predictor.predict(data_batch)
...
def detect_at_single_scale(predictor, data_names, imdb, test_data, cfg, thresh, vis, all_boxes_single_scale, logger,nms_dets):
    idx = 0
    ...
    for im_info, data_batch in test_data:
        ...
        scores_all, boxes_all, data_dict_all = im_detect(predictor, data_batch, data_names, scales, cfg, imdb=imdb)

        # boxes_all = boxes_all[0].astype('f')
        # scores_all = scores_all[0].astype('f')
...
def pred_eval(predictor, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True,nms_dets=None):
    """
    ...
    cache_extra_str = ''
    if predictor is None:
        cache_extra_str = '_docker'

    if hasattr(cfg,'test_idx'):
        det_file = os.path.join(imdb.result_path, imdb.name + '_{0}_detections'.format(cfg.test_idx)+ cache_extra_str+'.pkl')
    else:
        det_file = os.path.join(imdb.result_path, imdb.name + '_detections' + cache_extra_str + '.pkl')
    if os.path.exists(det_file) and not ignore_cache:
        with open(det_file, 'rb') as fid:
            all_boxes = cPickle.load(fid)
        info_str = imdb.evaluate_detections(detections=all_boxes,logger=logger)
        if logger:
    ...
    for test_scale_index, test_scale in enumerate(cfg.TEST_SCALES):
        det_file_single_scale = os.path.join(imdb.result_path, imdb.name + '_detections_' + str(test_scale_index) + cache_extra_str + '.pkl')
        ...
        detect_at_single_scale(predictor, data_names, imdb, test_data, cfg, thresh, vis, all_boxes_single_scale, logger,nms_dets)
    ...
    info_str = imdb.evaluate_detections(all_boxes,logger)
    if logger:
        logger.info('evaluate detections: \n{}'.format(info_str))
