...
class DataParallelExecutorGroup(object):
    ...
    def forward(self, data_batch, is_train=None):
        ...
        if LEONID_PROFILING_ENABLED:
            toc = time.time()
            print('DataParallelExecutorGroup::forward 1 {0}'.format(toc - tic))
            tic = time.time()
        ...
        if LEONID_PROFILING_ENABLED:
            toc = time.time()
            print('DataParallelExecutorGroup::forward 2  {0}'.format(toc - tic))
            tic = time.time()
        ...
        if LEONID_PROFILING_ENABLED:
            toc = time.time()
            print('DataParallelExecutorGroup::forward 3  {0}'.format(toc - tic))
